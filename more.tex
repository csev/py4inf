...
there is built-in support in Python which makes it very 
(i.e. sockets) easy to make network connections and retrieve data over those
sockets.

A socket is much like a file, except that it is a two-way connection between two 
programs.  You can both read and write the same socket.  If you write somthing to 
...
has not send any data - you just sit and wait.  And if the programs on both ends
...
and who sends next and so far.  In a sense the two applications at either end 
of the socket are doing a dance and makeing sure not to step on each other's toes.
...
find that to reqest a document from a web server, we make a connection to port 80, 
...
simple Python program that makes a connection to a web server and requests a document 
...
web pages and process the data in Python.   Using {\tt utllib}
...
examines the data in those pages lookig for patterns.
...
Google finds nearly all of the pages on the web (at least it 
finds those pages that are linked from some other page).   
Google also uses the frequency of links to a particular page
as one measure of how ``important'' a page is and how highly
...
Once it becomes easy to retrieve documents and parse documents 
...
the web is to exchage the data in a format called the ``eXtensible Markup 
Language'' or XML.  
...
for usr in dom.getElementsByTagName('name') :
    print 'Name:', usr.firstChild.nodeValue

for em in dom.getElementsByTagName('email') :
    print 'Email:', em.getAttribute('hide')
\end{verbatim}
\afterverb
%
The call to {\tt parseString} converts the string representation
of the XML into a {\bf Document Object Model} (or DOM).  When the
XML is in a DOM, we have a seties of methods which we can call to 
extract portions of data from the XML.  The DOM has the advantage
...
{\tt minidom} allows us not to worry about those rules.

The {\tt getElementsByTagName} function searches through the 
DOM and retrieves a list of {\bf nodes} that match the specified tag.
...
The code to pull out all of the elements which are in the {\tt name}
tag is as follows:
...
\afterverb
%
We wrte this as a {\tt for} loop because there may be multiple name
tags and the {\tt dom.getElementsByTagName} returns a list of nodes
even if it only finds a single node.  To retrieve the text in between
the open and close tags, we must go to the {\tt firstChild} node and
retrieve its ``value''.   The text within a node is modelled as a child
of the node.
...
\afterverb
%
It is somewhat easier to retrieve an attribute because attributes 
ate modelled at the node and can be retrieved using the
{\tt getAtribute} method:
...
\afterverb
%
This extracts the value of the hide attribute of the email node(s):
\beforeverb
\begin{verbatim}
Email: yes
\end{verbatim}
\afterverb
%
Many (including me) find using the DOM to extract data from XML a bit
``clunky'' and other Python packages such as {\bf ElementTree} gives
a more ``Pythonic'' way to parse XML.   The DOM has the advantage in that
once you can use the DOM, it is relatively similar across languages 
such as Python, Java, C++, etc.  These other languages also have DOM-style
XML parsers.

For the purposes of the examples in this book, we will stick with the
``classic'' DOM-based XML parsing approach - but as you do more work 
with XML you may want to explore these other XML parsers.
...
web site and bok air travel, hotels, and automobiles all from a 
...
makes a hotel reservation using the airline site, the airline site uses
...
A Servce-Oriented Approach has many advantages including: (1) we 
