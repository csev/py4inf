...
stored on permanent storage, it can be store far more data tha
...
amounts of data.   Databases maintain their performance by 
building \emph{indexes} as data is added to the database
to allow the database software to jump quickly to a particular
entry in the database.

\section{Database concepts}

When you first look at a database, it looks a lot like a a lot
of spreadsheets.   The primary data structures in a database are:
...
Using the browser, you can easily create tables, insert data
into the database, edit data in the database, or run simple SQL
...
many changes that you need ot do to a text file, often you 
...
or dictionaries\footnote{SQLite actualy does allow some 
...
Defining structure for your data up-front may seem inconvience
...
A {\tt cursor} is like a file handle that we can use to perform
...
Once we have the {\tt cursor}, we can begin to execute database
...
and over again without causeing an error.  Note that the
{\tt DROP TABLE} does drop the table and all of its contents
from the database.
...
\afterverb
%
The second command actually creates a table named
...
to retrieve the rows we just inserted rows from the table.  On the 
{\tt SELECT} command, we indicate whch columne we would like {\tt title, plays}
...
the database does not read all of the data from the
...
first value as the {\tt title} and the second value as the {\tt plays}.
...
that are capable of handing non-Latin character sets.
...
allows us to express a selection criterion so that we can as the database
to apply the command to the rows that match the criterion.  In this example
...
we also call {\tt commit()} to force the data to be written to the database.

\section{An example program using a database}

In this section, we will create a simple spidering program that will 
go through Twitter accounts and build a list of Twitter accounts.
...
You don't want to always start your data retrieval at the
beginning so we want to store data as we retrieve it so out
...
statuses and loop through the list of friends and add each 
...
Here is the source code to our Twitter spidering application:
...
account name, enter, or to quit the program.  If the user enters
a Twitter account, we retrieve the list of friends and statuses
for that user and add the friend to the database if the friend is 
not already in our list.  If the friend is already in the list, 
we add one the the {\tt friends} field in the row in the database.
...
and retreive the \verb"screen_name" for each user.  Then we use
the {\tt SELECT statement} to see if we already have stored this
particular \verb"screen_name" in the database.  We retrieve the
friend count if the record exists.
...
\afterverb
%
Once the cursor executes {\tt SELECT} statement, we must retrieve the rows.  We
could do this with a {\tt for} statement but since we are only retrieving
...
{\tt WHERE} clause to add one to the {\tt friends} column for the row which 
...
add a table named {\tt Twitter} to the database.  Then we retreive
...
to set the {\tt retreived} column to one to indicate that we have completed 
the retrieval of the friends of this account.  This keeps us from re-reteieving
the same data over and over and keeps us progressing forward through the network
of Twitter friends.

If we once again run the dumping program, it will give us the following output:
...
Since we now have retreived the friends of three people
...
unvisited account (i.e. the next account will be \verb"steve_coppin"),
...
Postgress, and SQLite.  We focus on SQLite in this book because
...
statement.  The {\tt WHERE} clause consists of logical operations that
determine which rows are to be deleted:
...
\afterverb
%
Note that unlike Python, in SQL we use a single equals sign 
to indicate a test for equality rather than a double equals sign.
Other locical operations allowed in a where clause include 
\verb"<",
\verb">",
\verb"<=",
\verb">=",
\verb"!=",
as well as {\tt AND} and {\tt OR} and parenthesis
...
the rows which are to be updated.  A single {\tt UPDATE} statement
can change all of the rows that match the {\tt WHERE} clause, or if 
a {\tt WHERE} clause is not specified, it performs the {\tt UPDATE}
on all of the rows in the table.

The last of the core SQL commands is the {\tt SELECT} statement
that allows you to retrieve fields from a table using an optional 
{\tt WHERE} clause as well as an {\tt ORDER BY} clause to control
...
Usinr \verb"*" indicates that you want to the database to return all of 
the columns for each row that matches the {\tt WHERE} clause.  You can request
that the returned rows be sorted by one of the fields as follows:
...
relationship between the two tables is called {\bf data modelling}.  The
design document that shows the tables and their relationships 
is called a {\bf data model}.

Data modelling is a relatively sophisticated skill and we will only introduce
the most basic concepts of relational data modeling in this section.  For more
detail on data modelling you can start with:

\url{http://en.wikipedia.org/wiki/Relational_model}

Lets say for our Twitter spider application, instead of just 
...
We since everyone will potentially have many acounts that follow
...
10 million people in our database and a possibility of a billion
friend links, it is important to be able to scan data as quickly
as possible.

The first thing that we do is to add an additional column to the 
{\tt Twitter} table to store the numeric key associated with the 
row.   SQLite has a feature which automatically adds the key value
for any row we insert into a table using a special kind of 
data column of type {\tt INTEGER PRIMARY KEY}.

We can create the {\tt Twitter} table with this additional 
column as follows:
...
the new program version of the program:
...
while True:
    ...
    for user in tree.findall('user'):
        ...
        try:
            ...
            countnew = countnew + 1
        cur.execute('''INSERT OR IGNORE INTO Friends 
...
the primary needed patterns which we need to make use of when we are
...
\item Insert the row that captures the ``friend'' relationship.
...
in each row of the {\tt Friends} table must be unique.  These constraints
...
``{\tt name} must be unique'' rule, the database system can just ignore the 
{\tt INSERT}.  We are using the database constraint as a safety net
to make sure we don't inadvertently do something dumb.

Similarly, the following code insures that we don't add the 
...
friend's account when we have extract a \verb"screen_name"
...
If all goes well\footnote{In general, when a sentance starts 
...
and contraol will transfer into the {\tt except} section.
...
Once we know the key value for both the Twiter user
and the friend in the XML, it is a simple matter to insert
the two numbers into the {\tt Friends} table
...
The following is the frist few rows in the {\tt People} 
and {\tt Friends} tables after this run is completed:
\beforeverb
\begin{verbatim}
People:
(1, u'drchuck', 1)
...
{\tt People} table and you see the numbers of the from and to ends of 
the {\tt Friends} table.   Looking at the data, you can see that 
``drchuck'' is a friend to all of the people.  This makes sense because
we started by reteieving all of the friends of ``drchuck'' from Twitter.

\section{Using JOIN to retrieve data}

Now that we have followed the rules of database normalization
and have data separated into two tables, we need to be able to build a 
...
\beforeverb
\begin{verbatim}
SELECT * FROM Friends JOIN People 
    ON Friends.to_id = People.id WHERE Friends.from_id = 2
\end{verbatim}
\afterverb
%
The {\tt JOIN} clause indicates that the fields we are selecting
cross both the {\tt Friends} and {\tt People} tables.  The {\tt ON}
clause indicates how the two tables are to be joined.   Take the rows
from {\tt Friends} and append the row from {\tt People} where the
field \verb"from_id" in {\tt Friends} is the same the {\tt id} value
in the {\tt People} table.

The result of the JOIN is to create an extra-long 'meta-row' which ahs both 
the fields from {\tt Friends} and the matching fields from {\tt People}.
...
and {\tt Friends} and then dump out a subset of the
...
\afterverb
%
You see the columns from the {\tt People} and {\tt Friends} tables and the last
...
see that the second column (\verb"Friends.to_id") matches the third column
