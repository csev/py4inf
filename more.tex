...
perform some operation on each file.  These scripts
can make short work of simple tasks that must be done to files
...
and subdirectories recursively.   The string ``.'' indiates
to start in the current directory and walk downard.
As it encounters each directory,
we get three values in a tuple.  The first value is the curent
...
We do not have to explicitly look into each of the sub-folders
because we can count on {\tt os.walk} to visit every 
folder eventually.  But we do want to look at each file so 
we write a simple {\tt for} loop to loop throu the files 
int he current directory.   We check each file to see if 
it ends with ``.txt'' and then count the number of 
files through the whole directory tree that end with the
suffix ``.txt''.

Once we have a sense of how many of these files, the next
thing to do is try to figure out a way to automatically
figure out in Python which files are bad and which files
are good.   So we write a simpe program to print out the
...
{\tt os.path.join} because on Windows we use a backslash
...
files are a generic bit of HTML that was came in from mail 
sent to my system from my T-Mobile phone:
...
in these files so we can probably delete them and perhaps we should
just delete all of the files that have any HTML in them since 
these files are supposed to be captions and not contain HTML.
...
\afterverb
%
We use a {\tt contiue} to skip files with the two 
