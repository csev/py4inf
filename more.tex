...
from the command line where we start Python.  Up to now,
...
\afterverb
%
We can place additional parameters after the Python file and access
those parameters in our Python program.  Here is a simple program 
that demonstrates reading parameters from the command line:
...
is the name of the Python program and the remaining strings are the parameters
on the command line after the Python file.

The following shows our program reading several parameters from the command
line:
...
the two parameters after the file name.

We can rewrite our program to read the file, taking the file name from the parameter
as follows:
\beforeverb
\begin{verbatim}
import sys
...
\afterverb
%
We take the second parameter as the name of the file (skipping past
...
\afterverb
%
Using parameters as input can make it easier to reuse your Python programs 
...
\item[argument:] Parameters on the command line after the Python file name.
...
You can download and install the BearutifulSoup code
from:
...
\chapter{Case Study: Spidering HTML and XML data}

In this chapter, we will bring a number of the concepts
of the previous chapters together to build a relatively 
sophisticated application.  Our purpose in this application
is to spider HTML and XML data from a Wiki system and 
produce activity reports sorted by page and by user.

We will build a simple web crawler using a database 
to pull the XML and RSS data and then parse the data 
extracting the activity information.

\section{Our web crawler}

\section{Retrieving RSS}
