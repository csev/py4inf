...
{\tt ElementTree} allows us not to worry about those rules.
...
of the use of Twitter is through the API (i.e. not through the Twitter
web user inteface).
...
If we were to dump out the retrieved XML it would look roughly as follows:
...
element with more sub-nodes we look deeper into the node unti we reach the 
...
\afterverb
%
While the code for this takes a few lines to express what we are looking for 
in the XML, it is much simpler than trying to use Python string parsing
to pull apart the XML and find the data elements we are looking for.
