...
And we wanted to pull out only the second half of the E-Mail address (i.e.
...
a position in the string where we want to {\tt find} to start looking.
When we slice, we extract the characters 
from ``one beyond the at-sign throught up to {\em but not including} the 
space character''.  
...
or the string after the colon character and then use the 
{\tt float} function to convert the extracted string 
int o a floating point number.
...
floating point number on the line.  Compute the count of these 
lines and the total of the spam confidence from these lines.
...
E-Mail addresses from the line.   Count the number of
