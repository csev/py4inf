...
query launguage using SQLIte3, web scraping, and calling REST-based
Application Program Interfaces using Twitter.

What is interesting is how much of the original {\em Think Python} book
...
\beforefig
\centerline{\includegraphics[height=1.50in]{figs2/arch.eps}}
\afterfig
...
{\bf object code}, {\bf machine language} or the {\bf executable}.  
...
\item[machine language:]  The lowest level language for software which 
...
the notion of words?  Beginning programmers have touble separating what parts of the
...
exception occurs in the {tt try} block, 
it jumps out of the {\tt try} block and
...
writing fruitful functions in the next chapter.
...
that takes an integer parameter and computes a different kind of sequence.
Remeber that the percent sign is the {\bf modulo} operator which gives us the
...
point numbers.  Since the main loop repeatedly divides a number
by two, a number in the billions converges to one in 
relatively few steps.  It is more fun to try floating point values 
...
These loops fall into several patterns.

\subsection{Counting loop}

% TBD

\subsection{Summing loop (total)}

% TBD

\subsection{Maximum loop}

% TBD

\subsection{Minimum loop}

% TBD

\subsection{Searching loop}
