...
In short, they represent every field outside of Computer Science.
...
\afterverb
%
Translating this look to English is not a direct as the 
...
If you think about the variable {\tt total}, contains the 
...
Neither the counting loop not the summing loop are particularly 
...
\afterverb
%
The variable {\tt largest} is the ``largest value we have seen so far''.
Before the loop, we set {\tt largest} to the contstant {\tt None}.  
{\tt None} is a special value which we can store in a variable to mark 
...
% TBD - Talk about the guardian pattern
