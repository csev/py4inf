...
Winter 2009.  The first 10 chapters of the book will be 
...
we take a line and map it all to lower case before we do any checking
...
\index{guardian pattern}
\index{pattern!guardian}
Another way to safely write the if statement using the {\bf guardian}
...
splitting the string yielded four words and the third word 
({\tt word[2]}) is ``nice''.
...
\afterverb
%
It kind of works and we see the day from the first from line
...
statement, we omit the parentheses, but the following is equally 
valid syntax:
...
Sarah Hegge for catching errors,
