...
library'' respectively.  In Linux and Maxintosh there is suffix that uniquely marks
...
Even out simple {\bf hello.py} script is a program.  It is a one-line
...
into a file called {\bf words.py} and run it you download the source 
...
to Python, instead we are communicating to each other {\em through Python}.
...
or store them in a file or erhaps wrtie them to a device like a
...
need it.  Its error messages sound harsh, by they are just Python's
...
\item[Logic errors:] A logic error is when your description of the steps to take 
is syntactically perfect, but there is a mistake in the directions.  A simple example would
...
they are on a farm and in a barn and completely lost.  The you say ``did you turn left
or right gas station?'' and they say, ``I followed your directions perfectly, I have 
...
sadly contained a small but undetected logic error.''. 
...
full understanding the details you can get a beter understanding of they ``why?'' 
...
if the material you are currently reading at seems a bit impenetrable.
...
I something seems particularly hard, there is usually no value in staying up all 
