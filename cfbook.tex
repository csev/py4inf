...
to determine which pages are most highly connected and then vizualize
the page rank and connectivity of our small corner of the web.
We will use the D2 JavaScript Visualization library 
\url{http://d3js.org/} to product the visualization output.
...
the page rank the more times you run it.  You can even run {\bf sprank.py} a few times
...
If you rerun the other utilities, re-run {\bf spjson.py} 
...
{\em So it is very important that you make use of the gmane.org data
responsibly by adding delays ot your access of their services and spreading
long-running jobs over a longer period of time.  Do not abuse this free service
and ruin it for the rest of us.}

When I spidered the Sakai email data using this software, it produced nearly 
a Gigabyte of data and I spread the download process over about five days.  
...
data cleaning process. This is a sample run of {\bf gmadel.py}.  It prints a line out each time
250 mail messages are processed so you can see some progress happening as this program may
run for a while processing nearly a Gigabyre of mail data.
...
The {\bf gmodel.py} program handles a number of data cleaing tasks.
...
DNS names you want mapped to a single DNS.  In the Sakai data I added the following
mapping:
...
and vizualize your data.

Youcan produce a simple vizualization of the word frequence in the subject lines
...
{\bf gword.htm} to produce a word cloud siilar to the one at the beginning 
of this section.
...
This is a relatively complex and sophisticated application and is pretty much
ready to do some heavy data retrieval and analysis from mail data using 
\url{gmane}.
